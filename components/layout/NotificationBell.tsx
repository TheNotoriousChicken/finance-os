  ...
  return (
    <div className="relative">
      <button
        ...
        className="w-9 h-9 rounded-xl flex items-center justify-center text-[#A1A1AA] hover:text-slate-200 hover:bg-white/8 transition-colors relative"
      >
        <Bell size={18} />
        ...
        <>
          ...
          <div className="absolute right-0 top-11 z-50 w-72 rounded-2xl overflow-hidden" style={{ background: '#111113', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 20px 60px rgba(0,0,0,0.8)' }}>
            <div className="px-4 py-3 border-b" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
              <p className="leading-relaxed text-[12px] font-bold text-[#52525B] uppercase tracking-widest">Notifications</p>
            </div>
            {notifications.map(n => (
              <div key={n.id} className="px-4 py-3 border-b hover:bg-white/3 transition-colors" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
                <p className="leading-relaxed text-[13px] font-normal text-slate-200">{n.title}</p>
                <p className="leading-relaxed text-[12px] text-[#52525B] mt-0.5">{n.body}</p>
              </div>
            ))}
          </div>
        </>
