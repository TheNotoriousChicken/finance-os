    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }} className=" max-w-4xl mx-auto pb-10 page-enter">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="leading-relaxed text-sm text-[#52525B] mt-1">Net worth and investment tracking</p>
        </div>
      ...
      <div className="premium-card rounded-3xl p-8 relative overflow-hidden group">
        ...
        <p className="leading-relaxed text-[12px] font-bold text-[#A1A1AA] uppercase tracking-widest mb-2">Total Net Worth</p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "16px", marginTop: "32px" }}>
          <div className="bg-white/[0.02] rounded-xl p-4 border border-white/[0.05]">
            <p className="leading-relaxed text-[11px] text-[#A1A1AA] uppercase tracking-wider mb-1">Total Assets</p>
            <p className="tabular-nums text-lg font-bold text-slate-200">{formatPaise(summary.totalAssetsPaise)}</p>
          </div>
          <div className="bg-white/[0.02] rounded-xl p-4 border border-white/[0.05]">
            <p className="leading-relaxed text-[11px] text-[#A1A1AA] uppercase tracking-wider mb-1">Total Liabilities</p>
            <p className="tabular-nums text-lg font-bold text-[#FF4757]">{formatPaise(summary.totalLiabilitiesPaise)}</p>
          </div>
          <div className="bg-white/[0.02] rounded-xl p-4 border border-white/[0.05]">
            <p className="leading-relaxed text-[11px] text-[#A1A1AA] uppercase tracking-wider mb-1">Unrealized P&L</p>
            <p className={`text-lg font-bold ${pnlPositive ? 'text-[#00D68F]' : 'text-[#FF4757]'}`}>
              {pnlPositive ? '+' : ''}{formatPaise(summary.unrealizedPnLPaise)}
              <span className="leading-relaxed text-xs ml-2 opacity-70">({pnlPositive ? '+' : ''}{summary.pnlPercentage.toFixed(2)}%)</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4 mt-4">
        <h3 className="leading-relaxed text-[13px] font-bold text-[#A1A1AA] uppercase tracking-widest px-1">Your Portfolio</h3>
        ...
        {assets.length === 0 ? (
          <div className="minimal-card rounded-2xl p-10 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-slate-200 font-normal mb-1">No assets tracked yet</p>
            <p className="leading-relaxed text-sm text-[#A1A1AA] max-w-xs">Add your first stock, mutual fund, or crypto to start tracking your net worth.</p>
          </div>
        ) : (
          <div className="minimal-card rounded-2xl overflow-hidden">
            <table className="leading-relaxed w-full text-left text-sm whitespace-nowrap">
              <thead className="leading-relaxed bg-[#121214] border-b border-[#27272A] text-[11px] uppercase tracking-wider text-[#A1A1AA]">
                <tr>
                  <th className="px-5 py-3 font-normal">Asset</th>
                  <th className="px-5 py-3 font-normal">Quantity</th>
                  <th className="px-5 py-3 font-normal">Avg Price</th>
                  <th className="px-5 py-3 font-normal">Current Price</th>
                  <th className="px-5 py-3 font-normal text-right">Total Value</th>
                  <th className="px-5 py-3 font-normal text-right">P&L</th>
                </tr>
              </thead>
                    ...
                    <tr key={asset.id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-5 py-4">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          {asset.ticker && <span className="leading-relaxed text-[11px] text-[#52525B]">{asset.ticker}</span>}
                        </div>
                      </td>
                      <td className="px-5 py-4 text-[#A1A1AA]">{asset.quantity.toLocaleString(undefined, { maximumFractionDigits: 6 })}</td>
                      <td className="tabular-nums px-5 py-4 text-[#A1A1AA]">{formatPaise(asset.averageBuyPricePaise ?? 0)}</td>
                      <td className="tabular-nums px-5 py-4 text-slate-200 font-normal">{formatPaise(asset.currentPricePaise)}</td>
                      <td className="tabular-nums px-5 py-4 text-right font-bold text-slate-200">{formatPaise(current)}</td>
                      <td className="px-5 py-4 text-right">
                        <p className={`font-normal ${isGain ? 'text-[#00D68F]' : 'text-[#FF4757]'}`}>
