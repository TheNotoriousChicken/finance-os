    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }} className=" max-w-4xl mx-auto pb-10 page-enter">
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="leading-relaxed text-sm text-[#52525B] mt-1">Your spending patterns at a glance</p>
        </div>
        {momChange !== 0 && (
      ...
      <div className="minimal-card rounded-2xl overflow-hidden">
        <div className="px-6 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <p className="leading-relaxed text-[11px] font-bold text-[#52525B] uppercase tracking-widest">Monthly Spending</p>
        </div>
      ...
      <div className="minimal-card rounded-2xl overflow-hidden">
        <div className="px-6 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <p className="leading-relaxed text-[11px] font-bold text-[#52525B] uppercase tracking-widest">By Category</p>
        </div>
        <div className="p-4 space-y-1">
          {sortedCats.length === 0 ? (
            <div className="leading-relaxed py-8 text-center text-[#52525B] text-sm">No data</div>
          ) : (
            sortedCats.map(([name, data]) => {
              ...
              return (
                <div key={name} className="flex items-center gap-4 px-3 py-3 rounded-xl hover:bg-white/[0.03] transition-colors">
                  ...
                  <span className="leading-relaxed text-sm font-normal text-[#D4D4D8] flex-1 truncate">{name}</span>
                  ...
                  <span className="leading-relaxed tabular-nums text-[13px] font-bold text-slate-200 w-20 text-right flex-shrink-0">{formatPaise(data.amount)}</span>
                  <span className="leading-relaxed text-[11px] text-[#52525B] w-10 text-right flex-shrink-0">{pct.toFixed(0)}%</span>
                </div>
              );
            })
          )}
